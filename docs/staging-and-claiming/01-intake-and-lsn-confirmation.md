...
| slot lost on failover (pre-PG-17), or the source database restored from a backup that carries no slot (`pg_basebackup`, PITR, `pg_dump`/`pg_restore`) | same shape as above | same as above. The pause holds until the operator has finished the recovery and chooses when, and in what order, to rebuild |
| source database restored from a cold file-level copy of the stopped cluster | the catalog, ring, progress row, targets and slot all roll back to the same point, so the slot sits where the restored progress row expects it | nothing to recover: intake carries on from the restored point with no pause. Trellis has no restore-specific handling; only the slot's fate distinguishes one kind of restore from another (issue #236) |
