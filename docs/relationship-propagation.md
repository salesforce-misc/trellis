...
3. **The composite-PK/`OneToOne`-target interaction is a halt, not a
   ...
   Filed as #177. Issue #163 was adjacent but different: it was about
   ...
   single-column-PK precondition — and it is now fixed (alongside #171's
   group-key twin of the same encoding contract), which leaves #177 itself
   untouched: the halting gate is about *which* entry point enforces the
   single-column-PK precondition, not about how a key is encoded.
