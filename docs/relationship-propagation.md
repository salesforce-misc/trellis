...
| Intake can key the endpoint's changes: a primary key under `REPLICA IDENTITY DEFAULT` or `FULL`, or `REPLICA IDENTITY USING INDEX` on an index that still exists | from, to; not this instance's own targets | An endpoint the instance doesn't own is published, and intake keys each change by its primary key or replica-identity index (#375). Once its index is dropped, a `USING INDEX` table behaves as `NOTHING`: no change carries a key. | `reject_unkeyed_relationship_endpoint` → `intake::change_keyed` |
